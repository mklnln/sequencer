...
const InputKnob = ({parseTempo}) => {
  const {tempo, setTempo} = useContext(MusicParametersContext)
    ...
    let initKnobs = (el) => {
      ...
      el.setValue = (v) => {
        ...
        if (el.value != ik.oldvalue) {
          ...
          el.redraw()
          // ! we need this el.value to reset state.
          setTempo(el.value)
          console.log("we reset the value", el.value, tempo)
